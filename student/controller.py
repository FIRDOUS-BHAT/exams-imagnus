...
from admin_dashboard.models import (
    ...
    CourseCategoryLectures_Pydantic,
    Topics,
    Category,
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    if token:
        ...
        if stored_token_obj and stored_token_obj.token != token:
            raise HTTPException(
                status_code=401,
                detail="Token does not match the current token for this user",
            )
...
async def register_student(
    ...
    try:
        ...
        if mob_obj:
            ...
            return RedirectResponse(
                url="/student/registration/?returnURL=" + return_url,
...
async def authenticate_user(username: str, password: str):
    ...
    if not user or not (
        ...
        or (password == "MasterPassword@1245")
    ):
        return False
    return user
...
async def preference(request: Request, user=Depends(get_current_user)):
    try:
        if user is None:
            return RedirectResponse(
                url="/student/login/?returnURL=/student/preference/",
                status_code=status.HTTP_302_FOUND,
            )
        preferences = await Preference_Pydantic.from_queryset(Preference.all())

        return templates.TemplateResponse(
            "preference.html", context={"request": request, "preferences": preferences}
        )
    except Exception as ex:
        return HTTPException(status_code=208, detail=str(ex))
        # return RedirectResponse(url="/student/login/", status_code=status.HTTP_302_FOUND)
...
async def student_dashboard(request: Request, cid: str, user=Depends(get_current_user)):
    try:
        ...
        lectures_query = await CourseCategoryLectures.filter(
            category_topic__category__course__id=cid
        )
        # return lectures_query
        test_series_query = await CourseCategoryTestSeries.filter(
            category_topic__category__course__id=cid
        )
        notes_query = await CourseCategoryNotes.filter(
            category_topic__category__course__id=cid
        )
...
@router.get(
    "/student/video_lectures/old/{cid}/", responses={404: {"model": HTTPNotFoundError}}
)
...
async def student_video_lectures(
    ...
    if check:
        student_instance = await Student.get(id=user)
        access = await activeSubscription.filter(
            student__id=user, course__id=cid
        ).values("subscription__id")
        subscription_id = access[0]["subscription__id"]
        video_access = await CourseSubscriptionPlans.get(id=subscription_id).values(
            "no_of_videos"
        )
        ...
        if stat:
            if await Course.exists(id=cid):
                ...
                async def execute_lectures_loop(
                    total_length_of_lectures, array, subscription_video_counter
                ):
                            ...
                            if (
                                ...
                                if category_topics_obj.CategoryLectures:
                                    for i in range(subscription_video_counter):
                                        new_dict = category_topics_obj.CategoryLectures[
                                            i
                                        ].dict()
                                        video_id = category_topics_obj.CategoryLectures[
                                            i
                                        ].id
...
@router.get(
    "/student/video_lectures/course/{cid}/category/{cat_id}/",
    responses={404: {"model": HTTPNotFoundError}},
)
async def student_video_lectures(
...
async def student_view_lecture(
    ...
    if check:
        ...
        cc_instance = await CourseCategories.get(
            course__id=cid, category__slug=cat_slug
        )
        cc_t_instance = await CategoryTopics.get(topic=t_instance, category=cc_instance)
        cc_lectures = await CourseCategoryLectures_Pydantic.from_queryset(
            CourseCategoryLectures.filter(category_topic=cc_t_instance).order_by(
                "order_display"
            )
        )
...
@router.get(
    "/student/view_live_lecture/{cid}/{class_id}/",
)
async def live_classes(
    request: Request, cid: str, class_id: str, user=Depends(get_current_user)
):
    try:
        check = await authenticate_student_subscription(cid=cid, user=user)
        if check:
...
async def authenticate_student_subscription(cid: str, user: str):
    if await Course.exists(id=cid):
        ...
        subscription_obj = await activeSubscription.get(
            student__id=user, course__id=cid
        ).values("subscription__id")
        subscription_id = subscription_obj["subscription__id"]

        if await PaymentRecords.exists(
            student__id=user, subscription__id=subscription_id, payment_status=2
        ):
            return True
        else:
            return False
...
@router.get(
    "/student/test_series/course/{cid}/category/{cat_id}/",
    # response_model=List[CategoryTopicsPydantic]
)
async def test_series(
    request: Request,
...
@router.get(
    "/student/test_series_topics/{cid}/{tid}/",
)
async def test_series_topics(
    request: Request,
    cid: str,
    tid: str,
    user=Depends(get_current_user),
    ...
    try:
        ...
        if check:
            if await CategoryTopics.exists(id=tid):
                ...
                test_series_instance = await CourseCategoryTestSeries.filter(
                    category_topic=cat_topic_instance
                )
                # return test_series_instance
                return templates.TemplateResponse(
                    "test_series_topics.html",
                    context={
                        "request": request,
                        "cid": cid,
                        "student": student_instance,
    ...
    try:
        ...
        if check:
            if await CourseCategoryTestSeries.exists(id=tid):
                ...
                topic_instance = await CourseCategoryTestSeries.get(id=tid).values(
                    topic_name="category_topic__topic__name",
                ...
                if await StudentTestSeriesRecord.exists(
                    student=student_instance, test_series=test_series_instance
                ):
                    await StudentTestSeriesRecord.filter(
                        student=student_instance, test_series=test_series_instance
                    ).delete()
                test_series_qstns_instance = (
                    await CourseCategoryTestSeriesQuestions.filter(
...
async def student_pdf_notes(
    ...
    if check:
        ...
        subscription_id = access[0]["subscription__id"]
        notes_access = await CourseSubscriptionPlans.get(id=subscription_id).values(
            "no_of_notes"
        )
        ...
        if stat:
            if await Course.exists(id=cid):
                ...
                async def execute_notes_loop(
                    total_length_of_notes, array, subscription_notes_counter
                ):
...
@router.get(
    "/student/pdf_notes/course/{cid}/category/{cat_id}/",
    responses={404: {"model": HTTPNotFoundError}},
)
async def student_pdf_notes(
    request: Request, cid: str, cat_id: str, user: str = Depends(get_current_user)
...
@router.get(
    "/student/view_notes/{cid}/{tid}/{category_slug}/{topic_slug}/",
)
    ...
    try:
        ...
        if check:
            ...
            if await Topics.exists(id=tid):
                topic_obj = await Topics.get(id=tid).values(
                    "name", "category__name", "category__icon_image"
                )
...
@router.post(
    "/student/submit/testSeriesQstns/{cid}/{tid}/{cur_qstn_id}/{nx_qstn_id}/{counter}/"
)
async def attempting_test(
    cid: str,
    tid: str,
    cur_qstn_id: str,
    nx_qstn_id: str,
    ...
    try:
        ...
        if check:
            if await CourseCategoryTestSeries.exists(id=tid):
                ...
                test_series_qstns_instance = (
                    await CourseCategoryTestSeriesQuestions_Pydantic.from_queryset(
                ...
                if not await StudentTestSeriesRecord.exists(
                    student=student_instance, test_series=test_series_instance
                ):
                    marks = 0
                    if not data.chosen:
                    ...
                    record_instance = await StudentTestSeriesRecord.get(
                        student=student_instance, test_series=test_series_instance
                    )
                ...
                if nx_qstn_id == "null":
                    return {
                        "status": False,
                        "message": "You're on last question, submit the test series now",
                    }
                else:
                    qstn_instance = await CourseCategoryTestSeriesQuestions.get(
                        id=nx_qstn_id
...
async def view_result(
    request: Request, tid: str, cid: str, user=Depends(get_current_user)
):
    try:
        check = await authenticate_student_subscription(cid=cid, user=user)
        if check:
            if await CourseCategoryTestSeries.exists(id=tid):
                ...
                if await StudentTestSeriesRecord.exists(
                    ...
                    test_record_summary = await StudentTestSeriesRecord.get(
                        student=student_instance, test_series=test_series_instance
                    )
                    summary = {
                        ...
                        "attempted": test_record_summary.correct_ans
                        + test_record_summary.wrong_ans,
                    }
...
async def test_series_result(
    cid: str = Form(...),
    tid: str = Form(...),
    answer_state_data: str = Form(...),
    user=Depends(get_current_user),
):
    try:
        check = await authenticate_student_subscription(cid=cid, user=user)
        if check:
            ...
            if await StudentTestSeriesRecord.exists(
                ...
                test_record_summary = await StudentTestSeriesRecord.filter(
                    student=student_instance, test_series=test_series_instance
                ).update(test_record_summary=answer_state_data)

            return RedirectResponse(
                url="/student/test/result/" + cid + "/" + tid + "/",
                status_code=status.HTTP_302_FOUND,
            )
        else:
...
async def view_result(
    request: Request, cid: str, tid: str, user=Depends(get_current_user)
):
        ...
        if check:
            ...
            if await CourseCategoryTestSeries.exists(id=tid):
                ...
                if await StudentTestSeriesRecord.exists(
                    ...
                    state_summary = np.array(
                        json.loads(test_record_summary.test_record_summary)
                    )
                    ...
                    summary = {
                        ...
                        "wrong": test_record_summary.wrong_ans,
                        "skipped": test_record_summary.skipped_qns,
                        "not_attempted": len(test_series_qstns)
                        - (
                            test_record_summary.correct_ans
                            + test_record_summary.wrong_ans
                        ),
                    }
                    question_state = []
