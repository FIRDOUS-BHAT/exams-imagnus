...
from scholarship_tests.pydantic_models import (
    rankPydantic,
    rankResponsePydantic,
    studentIdPydanctic,
    testRecordIn_Pydantic,
)
...
async def scholarship_testseries(data: studentIdPydanctic, _=Depends(get_current_user)):
    ...
    if await Student.exists(id=student_id):
        ...
        if await ScholarshipTestSeries.exists(
            lang=lang, course__id=course_id, on_date__lte=updated_at
        ):
            if not await StudentScholarshipTestSeriesRecord.exists(
                ...
                return JSONResponse(
                    {"status": True, "message": jsonable_encoder(test_obj)},
                    status_code=200,
                )
            else:
                return JSONResponse(
                    {"status": False, "message": "You've already attempted."},
                    status_code=208,
                )
        else:
            return JSONResponse(
                {"status": False, "message": "No Test available at this time"},
                status_code=208,
            )
...
async def add_test_records(data: testRecordIn_Pydantic, _=Depends(get_current_user)):
    ...
    if await Student.exists(id=student_id):
        ...
        if await ScholarshipTestSeries.exists(id=test_series_id):
            ...
            marks = (
                (test_series.total_marks) / (test_series.no_of_qstns)
            ) * correct_ans
            if not await StudentScholarshipTestSeriesRecord.exists(
                test_series=test_series, student=student
            ):
        ...
        return JSONResponse(
            {"status": False, "message": "Series doesn't exist."}, status_code=208
        )
...
async def update_start_time(test_series_id: str, start_time: datetime):
    obj = await ScholarshipTestSeries.filter(id=test_series_id).update(
        on_date=start_time
    )
...
async def update_announcement_date(test_series_id: str, announcement_time: datetime):
    obj = await ScholarshipTestSeries.filter(id=test_series_id).update(
        result_announcement_date=announcement_time
    )
    return {"date modified"}
...
async def get_students_rank(data: rankPydantic, _=Depends(get_current_user)):
    ...
    if await StudentScholarshipTestSeriesRecord.exists(
        ...
        updated_at = datetime.now(tz)
        if await ScholarshipTestSeries.exists(
            id=test_obj.test_series_id, result_announcement_date__lte=updated_at
        ):
            test_series_obj = await ScholarshipTestSeries.get(
                id=test_obj.test_series_id
            )
            test_series_questions = await ScholarshipTestSeriesQuestions.filter(
                test_series__id=test_obj.test_series_id
            )

            result_obj = await StudentScholarshipTestSeriesRecord.get(
                student__id=student_id, test_series__id=test_obj.test_series_id
            )
            rank_query = await StudentScholarshipTestSeriesRecord.filter(
                test_series__id=test_obj.test_series_id
            ).order_by("-marks", "time_taken")
            top_ten = (
                await StudentScholarshipTestSeriesRecord.filter(
                    test_series__id=test_obj.test_series_id
                )
                .order_by("-marks", "time_taken")
                .limit(10)
                .values(
                    ...
                    "student_id",
                    "test_series_id",
                    student_name="student__fullname",
                )
            )
            ...
            new_dict.update({"ScholarshipTestSeries": test_series_questions})
            new_dict.update({"ScholarshipTestSeries_student": result_obj})
            ...
            return JSONResponse(
                {"status": True, "message": "No data Found"}, status_code=208
            )
    else:
        return JSONResponse(
            {"status": False, "message": "No student data Found"}, status_code=208
        )
