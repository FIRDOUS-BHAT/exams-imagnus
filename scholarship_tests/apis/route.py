...
async def scholarship(data: studentIdPydanctic):
    ...
    lang = data.lang
    test_obj = await ScholarshipTestSeries.get(lang=lang)
    updated_at = datetime.now(tz)
    if await Student.exists(id=student_id):
        test_series_id = None
        student = await Student.get(id=student_id)
        if await StudentScholarshipTestSeriesRecord.exists(student=student, is_attempted=True):
            if await ScholarshipTestSeries.filter(lang=lang, result_announcement_date__lte=updated_at):
            ...
            if await ScholarshipTestSeries.filter(lang=lang, on_date__lte=updated_at, end_date__gte=updated_at):
                status = 'online'
                test_series_id = test_obj.id
            elif await ScholarshipTestSeries.filter(lang=lang, on_date__gt=updated_at):
                status = 'not_yet_started'
            elif await ScholarshipTestSeries.filter(lang=lang, end_date__lt=updated_at):
...
async def scholarship_testseries(data: studentIdPydanctic,  _=Depends(get_current_user)):
    ...
    if await Student.exists(id=student_id):
        ...
        if await ScholarshipTestSeries.exists(lang=lang, on_date__lte=updated_at):
            if not await StudentScholarshipTestSeriesRecord.exists(student=student, is_attempted=True):

                test_obj = await ScholarshipTestSeries_Pydantic.from_queryset_single(
                    ScholarshipTestSeries.get(lang=lang))
