...
request.onupgradeneeded = function (event) {
  ...
  var objectStore = db.createObjectStore("test_series", {
    keyPath: "id",
    autoIncrement: true,
  });
};
...
function add(counter, chosen) {
  var request = db
    .transaction(["test_series"], "readwrite")
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.status) {
        ...
        callback =
          "/student/submit/testSeriesQstns/" +
          data.cid +
          "/" +
          data.tid +
          "/" +
          data.qstn_content.id +
          "/" +
          nx +
          "/" +
          data.counter +
  ...
  fetch(callback, {
    ...
    .then((data) => {
      ...
      if (data.status) {
        ...
        $("#no_of_not_visited").text(
          parseInt(no_qstns) - (data.message.attempted + data.message.skipped)
        );
      }
    })
    .catch((error) => {
...
$(document).on("click", ".video_list", function (e) {
  ...
  if (url && title) {
    ...
    if (url.endsWith(".mp4")) {
      // Use the video tag
      $("#new_url").replaceWith(
        '<video id="new_url" src="' +
          url +
          '" controls style="border: none; position: absolute; top: 0; height: 80%; width: 100%;" allowfullscreen controlsList="nodownload"></video>'
      );
    } else if (url.includes("iframe.mediadelivery.net")) {
      // Use the iframe
      $("#new_url").replaceWith(
        '<iframe id="new_url" src="' +
          url +
          '" loading="lazy" style="border: none; position: absolute; top: 0; height: 100%; width: 100%;" allow="accelerometer; gyroscope; encrypted-media; picture-in-picture;" allowfullscreen="true"></iframe>'
      );
    }
