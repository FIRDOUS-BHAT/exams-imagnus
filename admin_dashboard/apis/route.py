...
# from fastapi_cache.decorator import cache
from scholarship_tests.models import (
    ScholarshipTestSeries,
...
@router.get(
    "/get_all_preferences/",
    response_model=List[Preference_Pydantic],
...
async def get_all_preferences(
    ...
    try:
        obj = await Preference_Pydantic.from_queryset(
            Preference.all().order_by("display_order")
        )
...
@router.get(
    "/get_each_preference_courses/{preference_slug}",
    response_model=Preference_Pydantic,
    responses={404: {"model": HTTPNotFoundError}},
)
# @cache(expire=60)
async def get_each_preference_courses(
    preference_slug: str, user=Depends(manager)
):
    try:
        ...
        return await Preference_Pydantic.from_queryset_single(
            Preference.get(slug=preference_slug)
        )
...
async def course_details(
    ...
    try:
        ...
        course_cat_obj = await CourseCategories_Pydantic.from_queryset(
            CourseCategories.filter(course__slug=c_slug).order_by(
    ...
    try:
        ...
        for category in course_cat_obj:
            ...
            lecture_count = await CourseCategoryLectures.filter(
                category_topic__category__course__slug=c_slug,
                category_topic__category__category__id=cat_id,
            ).count()
...
async def update_video_links(user=Depends(manager)):
...
async def update_video_urls():
    ...
    # query = Q(mobile_video_url__isnull=True)
    query = Q(video_360__contains=".b-cdn.net/")
...
async def getBookMarkedVideos(student_id):
    if await BookMarkedVideos.exists(student__id=student_id):
        video_ids = await BookMarkedVideos.filter(student__id=student_id).values(
            "video__id"
        )
...
async def getBookMarkedNotes(student_id):
    if await BookMarkedNotes.exists(student__id=student_id):
        notes_ids = await BookMarkedNotes.filter(student__id=student_id).values(
            "notes__id"
        )
...
@router.post(
    "/course_category/{course_slug}/{category_slug}/{student_id}/",
    #  response_model=CourseCategoryPydantic
)
async def get_course_category(
    course_slug: str, category_slug: str, student_id: str, user=Depends(manager)
):
    ...
    async def execute_lectures_loop(subscription_initial_video_counter):
        topics = (
            await CourseCategoryLectures.filter(
                category_topic__category__course__slug=course_slug,
                category_topic__category__category__slug=category_slug,
            )
            .group_by("category_topic__topic__id", "category_topic__order_seq")
            .order_by("category_topic__order_seq")
            .values(topic_id="category_topic__topic__id")
        )
        ...
        for topic in topics:
            allowed_lectures = []
            topic_obj = await Topics.get(id=topic["topic_id"]).values(
                "id", "name", "slug"
            )
            ...
            lectures_obj = (
                await CourseCategoryLectures.filter(
                    category_topic__category__course__slug=course_slug,
                    category_topic__category__category__slug=category_slug,
                    category_topic__topic__id=topic["topic_id"],
                )
            ...
            if subscription_initial_video_counter <= each_topic_lectures_length:
                ...
                not_allowed_lectures_length = (
                    each_topic_lectures_length - subscription_initial_video_counter
                )

                for j in range(not_allowed_lectures_length):
                    disallowed_dict = lectures_obj[
                        subscription_initial_video_counter + j
                    ]
    ...
    async def execute_notes_loop(initial_notes_counter):
        topics = (
            await CourseCategoryNotes.filter(
                category_topic__category__course__slug=course_slug,
                category_topic__category__category__slug=category_slug,
            )
            .group_by("category_topic__topic__id")
            .values(topic_id="category_topic__topic__id")
        )
        topics = np.array(topics)
        notes = []
        for topic in topics:
            ...
            notes_obj = (
                ...
                .prefetch_related(
                    "notes_studentNotesActivity", "students_bookmarked_notes"
                )
                .values("id", "title", "slug", "thumbnail", "notes_url")
            )
            ...
            if each_topic_notes_length <= total_length_of_notes:
                if initial_notes_counter <= each_topic_notes_length:
                    ...
                    not_allowed_notes_length = (
                        each_topic_notes_length - initial_notes_counter
                    )
                    ...
                    initial_notes_counter = (
                        initial_notes_counter - each_topic_notes_length
                    )
    ...
    async def execute_test_series_loop(subscription_test_series_counter):
        topics = (
            await CourseCategoryTestSeries.filter(
                category_topic__category__course__slug=course_slug,
                category_topic__category__category__slug=category_slug,
            )
            .group_by("category_topic__topic__id")
        ...
        for topic in topics:
            ...
            test_series_obj = (
                ...
                .prefetch_related(
                    "test_series_studentTestSeriesActivity",
                    "students_bookmarked_testseries",
                )
                .values(
                    "id",
                    "title",
            ...
            if subscription_test_series_counter <= each_topic_test_series_length:
                ...
                not_allowed_test_series_length = (
                    each_topic_test_series_length - subscription_test_series_counter
                )

                for j in range(not_allowed_test_series_length):
                    disallowed_dict = test_series_obj[
                        subscription_test_series_counter + j
                    ]
                ...
                subscription_test_series_counter = (
                    subscription_test_series_counter - each_topic_test_series_length
                )
    ...
    category_course_ins = await CourseCategories.get(
        course__slug=course_slug, category__slug=category_slug
    )
    ...
    if not category_course_ins.is_free:
        if await activeSubscription.exists(
            ...
            active_subscription = await activeSubscription.get(
                student__id=student_id, course__slug=course_slug
            ...
            if await StudentChoices.exists(
                ...
                student_choice = await activeSubscription.get(
                    student__id=student_id, course__slug=course_slug
                ).values(
                    "subscription__no_of_videos",
                    "subscription__no_of_notes",
                    "subscription__no_of_tests",
                    "subscription__live_classes_access",
                )
                ...
                live_classes_access = student_choice[
                    "subscription__live_classes_access"
                ]
            ...
            updated_access_notes_array = await execute_notes_loop(initial_notes_counter)
        ...
        updated_access_lect_array = await execute_lectures_loop(initial_video_counter)
...
@router.post(
    "/v1/course_category/{course_slug}/{category_slug}/{student_id}/",
    #  response_model=CourseCategoryPydantic
)
# @cache(expire=cache_time)
async def course_category(
    course_slug: str, category_slug: str, student_id: str, user=Depends(manager)
):
    try:
        ...
        if await Student.exists(id=student_id):
            ...
            async def watch_activity(video_id):
                if await studentActivity.exists(
                    ...
                    if await studentVideoActivity.exists(
                        student_activity__student__id=student_id,
                        student_activity__course__slug=course_slug,
                        video_id__id=video_id,
                    ):
                        activity_instance = await studentVideoActivity.filter(
                            student_activity__student__id=student_id,
                            student_activity__course__slug=course_slug,
                            video_id__id=video_id,
                        ).values("watch_time")
            ...
            async def notes_activity(note_id):
                if await studentActivity.exists(
                    ...
                    activity_instance = await studentActivity.get(
                        student_activity__student__id=student_id,
                        student_activity__course__slug=course_slug,
                    )
                    notes_instance = await CourseCategoryNotes.get(id=note_id)
                    if await studentNotesActivity.exists(
                        student_activity__student__id=student_id,
                        student_activity__course__slug=course_slug,
                        note_id__id=note_id,
                    ):
                        notes_activity_instance = await studentNotesActivity.get(
                            student_activity=activity_instance, note_id__id=note_id
                        )
            ...
            async def testseries_activity(testseries_id):
                if await studentActivity.exists(
                    ...
                    if await studentTestSeriesActivity.exists(
                        student_activity__student__id=student_id,
                        student_activity__course__slug=course_slug,
            ...
            obj = await CourseCategories.exists(
                course__slug=course_slug, category__slug=category_slug
            )

            if obj:
                course_cat_obj = await CategoryTopics.filter(
                    category__course__slug=course_slug,
                ...
                async def execute_lectures_loop(array, subscription_video_counter):
                    ...
                    for category_topics_obj in array:
                        ...
                        if total_length_of_lectures > subscription_video_counter:
                            ...
                            new_lect_dict = {
                                "topic": await Topics.get(
                                    id=category_topics_obj.topic_id
                                )
                            }
                            ...
                            if (
                                ...
                                if category_topics_obj.CategoryLectures:
                                    if forward_access_flag:
                                        for i in range(access_counter):
                                            ...
                                            is_bookmarked = (
                                                await check_isBookmarkedVideo(video_id)
                                            )
                                            new_dict.update(
                                ...
                                if category_topics_obj.CategoryLectures:
                                    if forward_access_flag:
                                        for (
                                            eachLecture
                                        ) in category_topics_obj.CategoryLectures:
                ...
                async def execute_notes_loop(array, subscription_notes_counter):
                    ...
                    for category_topics_obj in array:
                        ...
                        if total_length_of_notes > subscription_notes_counter:
                            new_notes_dict = {
                                "topic": category_topics_obj.topic.dict(
                                    exclude={"category"}
                                )
                            }
                            # category_topics_array.append(
                            ...
                            if (
                                ...
                                if category_topics_obj.CategoryNotes:
                                    if forward_access_flag:
                                        for i in range(access_counter):
                                            ...
                                            notes_id = (
                                                category_topics_obj.CategoryNotes[i].id
                                            )
                                            is_bookmarked = (
                                                await check_isBookmarkedNotes(notes_id)
                                            )
                                            new_dict.update(
                                                {"isBookmarked": is_bookmarked}
                                            )
                                ...
                                if category_topics_obj.CategoryNotes:
                                    if forward_access_flag:
                                        for (
                                            eachNote
                                        ) in category_topics_obj.CategoryNotes:
                ...
                async def execute_test_series_loop(array, subscription_series_counter):
                    ...
                    for category_topics_obj in array:
                        ...
                        if total_length_of_test_series > subscription_series_counter:
                            ...
                            if (
                                len(category_topics_obj.CategoryTestSeries)
                                > subscription_series_counter
                            ):
                                for i in range(subscription_series_counter):
                                    new_dict = category_topics_obj.CategoryTestSeries[
                                        i
                                    ].dict(exclude={"CategoryTestSeriesQuestions"})
                                    test_series_id = (
                                        category_topics_obj.CategoryTestSeries[i].id
                                    )
                            ...
                            if subscription_series_counter and (
                                ...
                                if category_topics_obj.CategoryTestSeries:
                                    for (
                                        ...
                                        is_bookmarked = (
                                            await check_isBookmarkedTestSeries(
                                                test_series_id
                                            )
                                        )
                                        new_dict.update(
...
@router.post(
    "/v0/course_category/{course_slug}/{category_slug}/{student_id}/",
    response_model=CourseCategoryPydantic,
)
# @cache(expire=cache_time)
async def course_category(
        ...
        if await Student.exists(id=student_id):
            ...
            async def watch_activity(video_id):
                if await studentActivity.exists(
                    ...
                    videoActivity_instance = await studentActivity.get(
                        student=student_instance, course=course
                    )
                    video_instance = await CourseCategoryLectures.get(id=video_id)
                    if await studentVideoActivity.exists(
                        student_activity=videoActivity_instance, video_id=video_instance
                    ):
                        activity_instance = await studentVideoActivity.filter(
                            student_activity=videoActivity_instance,
            ...
            async def notes_activity(note_id):
                if await studentActivity.exists(
                    ...
                    notes_instance = await CourseCategoryNotes.get(id=note_id)
                    if await studentNotesActivity.exists(
                        student_activity=activity_instance, note_id=notes_instance
                    ):
                        notes_activity_instance = await studentNotesActivity.get(
                            student_activity=activity_instance, note_id=notes_instance
                        )
            ...
            async def testseries_activity(testseries_id):
                if await studentActivity.exists(
                    ...
                    testseries_instance = await CourseCategoryTestSeries.get(
                        id=testseries_id
                    )
                    if await studentTestSeriesActivity.exists(
                        student_activity=activity_instance,
            ...
            obj = await CourseCategories.exists(course=course, category=category)
                ...
                if not category_course_ins.is_free:
                    if await activeSubscription.exists(
                        ...
                        if await StudentChoices.exists(
                            ...
                            updated_access_test_series_array = (
                                await execute_test_series_loop(
                                    resp, subscription_initial_test_series_counter
                                )
                            )
...
@router.get(
    "/subscription_plans/course/{course_slug}/{student_id}/",
...
async def subscription_plans(
    ...
    if await Student.exists(id=student_id):
        ...
        for subscription in course_plans:
            subscription_instance = await CourseSubscriptionPlans.get(
                id=subscription.id
            )
            # now = datetime.today()
            tz = pytz.timezone("Asia/Kolkata")
            ...
            if await StudentChoices.exists(
                ...
                if existing_validity and (validity > existing_validity):
                    ...
                    updated_dict.update({"validity": new_validity})
                    updated_dict.update({"plan_price": new_price})
                initial_video_count = updated_dict["no_of_videos"]
                initial_notes_count = updated_dict["no_of_notes"]
                initial_tests_count = updated_dict["no_of_tests"]
                updated_dict.update(
        ...
        return JSONResponse(
            {"status": False, "message": "student not found"}, status_code=208
        )


@router.get(
    "/course/course_overview/{course_slug}/", response_model=List[CoursePydantic]
)
async def course_overview(course_slug: str, user=Depends(manager)):
    course = await Course.get(slug=course_slug)

    overview = await Course_Pydantic.from_queryset(Course.filter(slug=course_slug))
...
async def add_instructor(data: InstructorIn_Pydantic, user=Depends(manager)):
...
@router.get(
    "/get_live_classes/{course_slug}/", response_model=List[PNLiveClassesPydantic]
)
async def get_live_classes(
    course_slug: str,
):
    obj = await LiveClasses_Pydantic.from_queryset(
        LiveClasses.filter(course__slug=course_slug, is_paid=True).order_by(
            "-created_at"
        )
    )
    return obj
...
@router.get(
    "/get_live_classes/v2/{course_slug}/{student_id}/",
...
@router.get(
    "/get_filtered_live_classes/{course_slug}/{student_id}/{d}",
    response_model=List[LiveClassesPydantic],
)
async def get_live_classes(
    course_slug: str, student_id: str, d: str, user=Depends(manager)
):
    try:
        # parse d as date
        ...
        subscription = await activeSubscription.exists(
            course__slug=course_slug, student__id=student_id
        )
        if subscription:
            obj = await LiveClasses_Pydantic.from_queryset(
                LiveClasses.filter(course__slug=course_slug).filter(
                    streaming_time__range=[dt, dt1]
                )
            )
        else:
            obj = await LiveClasses_Pydantic.from_queryset(
                LiveClasses.filter(course__slug=course_slug, is_paid=False).filter(
                    streaming_time__range=[dt, dt1]
                )
            )
...
async def get_all_live_classes(user=Depends(manager)):
...
async def delete_live_classes(user=Depends(manager)):
    obj = await LiveClasses.all().delete()
    return obj


@router.delete("/delete_subscriptions/")
async def delete_all_subscriptions(user=Depends(manager)):
    await CourseSubscriptionPlans.all().delete()
    return {"all deleted"}


@router.post("/app_static_urls/")
async def add_static_urls(title: str, url: str, user=Depends(manager)):
    if not await addAppStaticUrls.exists(title=title):
        await addAppStaticUrls.create(title=title, url=url)
        return JSONResponse({"status": True, "message": "Submitted"}, status_code=201)


@router.get("/app_static_urls/")
async def static_urls(user=Depends(manager)):
    obj = await addAppStaticUrls_Pydantic.from_queryset(addAppStaticUrls.all())
    arr = []
    for stats in obj:
...
async def offer_banners(
    ...
    if not await offerBanners.exists(title=title):
        image_url = await upload_images(
            s3, folder="offer_banners", image=banner, mimetype=None
        )
...
async def offer_banners():
    obj = await offerBanners_Pydantic.from_queryset(offerBanners.all())
...
async def add_coupon(
    ...
    try:
        if subscription == "all":
            subscriptions = await CourseSubscriptionPlans.filter(is_active=True).values(
                "id"
            )
            for subscription in subscriptions:
                if not await Coupons.exists(
                    name=name.strip(), subscription__id=subscription["id"]
                ):
                    subscription = await CourseSubscriptionPlans.get(
                        id=subscription["id"]
                    )
            ...
            if not await Coupons.exists(
                name=name.strip(), subscription__id=subscription.strip()
            ):
                subscription = await CourseSubscriptionPlans.get(
                    id=subscription.strip()
                )
                await Coupons.create(
                    name=name.strip(),
                    discount=discount,
                ...
                return JSONResponse(
                    {"status": False, "message": "This coupon already exists"},
                    status_code=208,
                )
...
async def dashboard_count(data: DashboardPydantic, user=Depends(manager)):
    student_id = data.student_id
    course_count = await activeSubscription.filter(student__id=student_id).count()
    studyMaterial_count = await StudyMaterialOrderInstance.filter(
        student__id=student_id
    ).count()
...
async def update_video_id(user=Depends(manager)):
    ...
    for item in obj["items"]:
        ...
        await CourseCategoryLectures.filter(video_id=item["guid"]).update(
            video_duration=item["length"]
        )
        i = i + 1
    # show values
...
async def checkSchMobile(mobile: str, user=Depends(manager)):
    if await Scholarship2021.exists(mobile=mobile):
        return JSONResponse(
            {"status": False, "message": "Mobile Number is already registered."},
            status_code=200,
        )
    else:
        return JSONResponse(
            {"status": True, "message": "Mobile Number not registered."},
            status_code=200,
        )


@router.post(
    "/scholarship_form_2021/",
)
async def fill_scholarship2021(
    name: str, email: str, mobile: str, user=Depends(manager)
):
    if await Scholarship2021.exists(email=email):
        return JSONResponse(
            {"status": False, "message": "Email Id is already registered."},
            status_code=200,
        )
        ...
        return JSONResponse(
            {
                "status": True,
                "message": "You've successfully registered for Scholarship Test.",
            },
            status_code=200,
        )
